...
export async function getStudents(courseId) {
  const r = await fetch(`${API}/api/classes/${courseId}/students`, { credentials: 'include' });
  return r.json();
}
export async function getAssignments(courseId) {
  const r = await fetch(`${API}/api/classes/${courseId}/assignments`, { credentials: 'include' });
  return r.json();
}
...
export async function getHistory(courseId, studentId) {
  const r = await fetch(`${API}/api/participation/history?courseId=${courseId}&studentId=${studentId}`, { credentials: 'include' });
  return r.json();
}
